...
def playParticipateSound(card):
   if re.search(r'Capital Ship',card.Traits): playSound('participate_capital_ship1')
   elif re.search(r'Fighter',card.Traits):
      if card.Side == 'Light': playSound('participate_fighter_LS1')
      else: playSound('participate_fighter_DS1')
   elif re.search(r'Force User',card.Traits): playSound('participate_force_user1')
   elif re.search(r'Character',card.Traits): playSound('participate_character{}'.format(rnd(1,2)))
   elif re.search(r'Creature',card.Traits): playSound('participate_creature{}'.format(rnd(1,2)))
   elif re.search(r'Droid',card.Traits): playSound('participate_droid1')
   elif re.search(r'Vehicle',card.Traits): playSound('participate_vehicle1')
   
def playStrikeSound(card):
   if re.search(r'Wookiee',card.name) or card.name == 'Chewbacca': playSound('strike_wookie1')
   elif re.search(r'Capital Ship',card.Traits): playSound('strike_capital_ship1')
   elif re.search(r'Fighter',card.Traits):
      if card.Side == 'Light': playSound('strike_fighter_LS1')
      else: playSound('strike_fighter_DS1')
   elif re.search(r'Force User',card.Traits): playSound('strike_force_user{}'.format(rnd(1,2)))
   elif re.search(r'Trooper',card.Traits): playSound('strike_trooper1')
   elif re.search(r'Droid',card.Traits): playSound('strike_droid{}'.format(rnd(1,3)))
   elif re.search(r'Vehicle',card.Traits): playSound('strike_vehicle{}'.format(rnd(1,3)))
   elif re.search(r'Character',card.Traits): playSound('strike_character{}'.format(rnd(1,4)))
   elif re.search(r'Creature',card.Traits): playSound('strike_creature1')
